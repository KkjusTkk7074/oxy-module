...
const ShoukakuQueue = require('./OxyQueue.js');
const ShoukakuRest = require('./OxyRest.js');
const ShoukakuPlayer = require('../guild/OxyPlayer.js');
const ShoukakuStats = require('../struct/OxyStats.js');
 ...
 */
class OxySocket extends EventEmitter {
    /**
    ...
    constructor(oxy, options) {
        ...
        * @type {ShoukakuRest}
        */
        this.rest = new ShoukakuRest(options, oxy.options);
        /**
  ...
  /*async addq(guild,member,channel,node,track){
     
     const k = await this.players.get(guild.id) 
  / const existing = this.get(guild.id);
 //  if(!existing)
 if(!k) this.joinChannel({
                guildId: guild.id,
  ...
  //if(!x.playing)  return this.players.get(guild.id).playTrack(this
//.players.get(guild.id).Queue.shift())
...
//  }
  ...
  qp(guild){
    ...
    const trock =  this.players.get(guild).player.queue.shift()
    
    this.players.get(guild.id).player.playTrack(current)
  }
    /**
