...
const { mergeDefault } = require('../../Utils/util.js');
//const db = require("quick.db")
const ShoukakuConnection = require('./OxyConnection.js');
const ShoukakuTrack = require('../struct/OxyTrack.js');
const ShoukakuFilter = require('../struct/OxyFilter.js');
const OxySocket = require('../node/OxySocket.js');


const OxyTrackQueue = require('.././node/OxyTrackQueue.js')
/**
 * ShoukakuPlayer, used to control the player on a guild
 * @class ShoukakuPlayer
 */
class OxyPlayer extends EventEmitter { 
    /**
...
const { Client } = require("youtubei");
 const node = this.connection.node

const youtube = new Client();

const hmm =  await youtube.getVideo(this.previous.info.identifier)

const lmao =  await hmm.related
const gg = Math.floor(Math.random() * 19)

const url = await lmao[gg].title
const tf =  await node.rest.resolve(url,"youtube")


const trackm =  await tf.tracks.shift()

 this.Queue.add(trackm)
this.play()
   }
